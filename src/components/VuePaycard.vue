...
export default {
  ...
  computed: {
    ...
    cardType () {
      ...
      // add image brand
      re = new RegExp('^4(026|17500|405|508|844|91[37])')
      if (number.match(re)) return 'visaelectron'

      // add image brand
      re = new RegExp('^(?:5[0678]\d\d|6304|6390|67\d\d)\d{8,15}$')
      if (number.match(re)) return 'maestro'

      // add image brand
      re = new RegExp('^(5078\d{2})(\d{2})(\d{11})$')
      if (number.match(re)) return 'aura'
